 ...
 *   alexa-ai 2.0.0; the media/alias fixes from 2.1.0. `getEngine()` checks
 ...
 *       node -e "console.log(require('alexa-ai').version)"   # must print >= 2.1.0
 *
 * REQUIRED .env
...
const MIN_ENGINE_VERSION = "2.1.0";
...
/**
 * `.search <query>` — one-off web search, no memory writes.
 * `{ ok, text, sources: [{ title, url }], error, message }`
 */
