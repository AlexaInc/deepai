...
class IdentityGuard {
     ...
     * @returns {string}
     */
    sanitise(reply, wasIdentityQuestion = false) {
        let text = String(reply ?? '');
        if (!text.trim()) return text;

        const dirty =
            IdentityGuard.test(IdentityGuard.FORBIDDEN, text) ||
        ...
        text = text.replace(this.WRONG_MAKER, `created by ${this.creator}`);
    ...
    static sanitise(reply, wasIdentityQuestion = false) {
        return IdentityGuard.default.sanitise(reply, wasIdentityQuestion);
    }
